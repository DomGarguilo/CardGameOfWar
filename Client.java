...
class Client {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (Scanner clientInputScanner = new Scanner(System.in)) {
            host = Client.getHost(clientInputScanner);
            port = Utils.getPort(clientInputScanner, port);
            autoFlip = getAutoFlip(clientInputScanner);

            // initializing the client socket, output to server and input from server
            try (Socket clientSocket = new Socket("localhost", Client.port);
                 DataOutputStream outToServer = new DataOutputStream(clientSocket.getOutputStream());
                 InputStreamReader inputStreamReader = new InputStreamReader(clientSocket.getInputStream());
                 BufferedReader inFromServer = new BufferedReader(inputStreamReader)) {

                sendMsgToServer("Client to server test" + "\n", outToServer); // output test
                System.out.print("\nClient to server test msg sent\n");
                response = receive(inFromServer); // input test
                ...
                while ((response = receive(inFromServer)) != null) {
                    ...
                    if (response.contains(PRESS_ENTER)) {
                        ...
                        sendMsgToServer(input, outToServer);
     ...
     * @param outputStream DataOutputStream to use to send the message
     */
    private static void sendMsgToServer(String message, DataOutputStream outputStream) throws IOException {
     ...
     */
    private static String receive(BufferedReader bufferedReader) throws IOException {
        return bufferedReader.readLine();
    }

    // gets port from user
    public static String getHost(Scanner inputScanner) {
        ...
        return Objects.equals(input, "0") ? Client.host : input;
    }

    /**
     * Prompt the user if they want to enable automatic flipping of cards
     *
